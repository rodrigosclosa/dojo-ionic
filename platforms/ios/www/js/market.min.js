...
!function(app){app.config(function($stateProvider,$urlRouterProvider){$stateProvider.state("introduction",{url:"/market/intro",templateUrl:"templates/public/welcome.html",controller:"WelcomeCtrl"}),$stateProvider.state("authentication",{url:"/market/auth",templateUrl:"templates/public/authentication.html",controller:"AuthenticationCtrl"}),$stateProvider.state("private",{url:"/market/private",templateUrl:"templates/private/menu.html",controller:"MainCtrl"}),$urlRouterProvider.otherwise("/market/intro")})}(market);
!function(app){app.controller("AuthenticationCtrl",function($scope,$state,UserService,$ionicLoading){$scope.googleSignIn=function(){$ionicLoading.show({template:"Logging in..."}),window.plugins.googleplus.login({},function(user_data){UserService.setUser({userID:user_data.userId,name:user_data.displayName,email:user_data.email,picture:user_data.imageUrl,accessToken:user_data.accessToken,idToken:user_data.idToken}),$ionicLoading.hide(),$state.go("app.home")},function(msg){$ionicLoading.hide()})},$scope.introducao=function(){$state.go("introduction")}})}(market);
!function(app){app.controller("MainCtrl",function($scope,$state,UserService,$ionicLoading){$scope.user=UserService.getUser(),$scope.showLogOutMenu=function(){$ionicActionSheet.show({destructiveText:"Logout",titleText:"Are you sure you want to logout? This app is awsome so I recommend you to stay.",cancelText:"Cancel",cancel:function(){},buttonClicked:function(index){return!0},destructiveButtonClicked:function(){$ionicLoading.show({template:"Logging out..."}),window.plugins.googleplus.logout(function(msg){console.log(msg),$ionicLoading.hide(),$state.go("introduction")},function(fail){console.log(fail)})}})}})}(market);
